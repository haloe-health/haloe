...
- `book.html` — the 4-step booking wizard (Treatment → Date & Time → Details → Confirm) and **the only path that charges money**. State lives in one in-memory `state` object; panels toggle via `.panel.active` and `goToStep(n)`. The `SERVICES` constant is **the source of truth for treatments, durations, prices and descriptions**. Every treatment carries a `desc` (one compliant wellness line — see Compliance); packages carry theirs too.
